/* =========================================================
   SYNDILEX — config.js
   SOURCE UNIQUE des coordonnées affichées sur le site.

   Pour changer l'e-mail ou le téléphone de contact PARTOUT sur
   le site (pied de page, page Contact, mentions légales, et
   adresse de réception du formulaire), il suffit de modifier
   les valeurs ci-dessous puis d'enregistrer ce fichier.

   Ne rien modifier d'autre : ce fichier ne contient que des
   coordonnées, aucune information sensible ni mot de passe.
   ========================================================= */
window.SYNDILEX_CONTACT = {
  email: "syndilex.conseil@gmail.com",
  phoneDisplay: "06 83 27 36 95",
  phoneHref: "+33683273695",
  site: "https://syndilex.fr"
};
